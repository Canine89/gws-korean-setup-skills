...
명령 실행 중 선택이 필요한 항목을 사용자에게 안내한다:

1. **GCP 프로젝트**: 기존 프로젝트를 선택하거나 새 이름을 입력
2. **OAuth 동의 화면**:
   - **앱 이름**: 아무 이름이나 괜찮다 (예: "GWS CLI")
   - **사용자 유형**: 회사 Google Workspace 계정이면 **Internal**, 개인 Gmail이면 **External**
   - External을 선택하면 **테스트 사용자**에 본인 이메일을 등록해야 한다
